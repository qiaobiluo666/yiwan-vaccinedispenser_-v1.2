...
public final class NettyClient {
    ...
    public void connect() {
        bootstrap.connect(host, port).addListener((ChannelFutureListener) future -> {
            ...
            if (!future.isSuccess()) {
                ...
                socketChannel = (SocketChannel) future.channel();
                online = true;
                log.info("当前客户端：{} 成功连接到服务器 {}:{}", name, host, port);
        ...
        if (schedulerStarted.compareAndSet(false, true)) {
            sendScheduler.scheduleAtFixedRate(() -> {
                try {
                    ...
                    log.error("[SEND][{}] 发送调度异常: {}", name, e.getMessage(), e);
                }
    ...
    public Result send(byte[] msg, int frame, CabinetConstants.Cabinet cabinetType) {
        if (!online) {
            log.info("{}断开连接",cabinetType.name);
    ...
    public void onDisconnected() {
        this.online = false;
        sendQueue.clear();
        log.warn("[{}] 已掉线，暂停发送任务并清空发送队列", name);
    }
