...
import static com.yiwan.vaccinedispenser.core.common.emun.CabinetConstants.CabinetBType.*;
...
@Service
@Slf4j
public class CabinetBServiceImpl implements CabinetBService {

    @Autowired
    private NettySendService nettySendService;

    @Override
    public void apply(CabinetBApplyRequest request) {
        //当前帧号
        Integer frameNumberNow = FrameNumberConfig.getFrameNumber();
        // 选择什么柜子
        int cabinet = CabinetConstants.Cabinet.CAB_B.num;
        // 0x01 自动上药
        int type = CabinetConstants.CabinetBType.APPLY.num;
        //指令
        ...
        //获取请求头、数据长度
        StringBuilder stringBuilder =NettyUtils.toHandler(CabinetConstants.CabinetDataLength.APPLY.dataLength);
        ...
        log.info("{}柜-{}：{}", CabinetConstants.Cabinet.CAB_B.desc, CabinetConstants.CabinetBType.APPLY.desc,HexUtil.format(stringBuilder.toString().toUpperCase()));
        //给B控制板 发送消息
        nettySendService.sendMsg(request.getWorkMode(),bytes,frameNumberNow);
    }

    @Override
    public void getDistance(CabinetBGetDistanceRequest request) {
        //当前帧号
        Integer frameNumberNow = FrameNumberConfig.getFrameNumber();
        // 选择什么柜子
        int cabinet = CabinetConstants.Cabinet.CAB_B.num;
        // 0x01 自动上药
        int type = CabinetConstants.CabinetBType.DISTANCE.num;
        //指令
        int command = request.getCommand().num;
        //模式
        ...
        log.info("{}柜-{}：{}", CabinetConstants.Cabinet.CAB_B.desc, CabinetConstants.CabinetBType.DISTANCE.desc,HexUtil.format(stringBuilder.toString().toUpperCase()));
        //给B控制板 发送消息
        nettySendService.sendMsg(request.getWorkMode(),bytes,frameNumberNow);
    }
    @Override
    public void servo(CabinetBServoRequest request) {
        ...
        int cabinet = CabinetConstants.Cabinet.CAB_B.num;
        // 0x02 皮带伺服
        int type = CabinetConstants.CabinetBType.SERVO.num;
        //指令
        int command = request.getCommand().num;
        //模式
        int mode = request.getMode();
        ...
        log.info("{}柜-{}：{}", CabinetConstants.Cabinet.CAB_B.desc, CabinetConstants.CabinetBType.SERVO.desc,HexUtil.format(stringBuilder.toString().toUpperCase()));
        //给B控制板 发送消息
        nettySendService.sendMsg(request.getWorkMode(),bytes,frameNumberNow);
    }

    @Override
    public void step(CabinetBStepRequest request) {
        //当前帧号
        Integer frameNumberNow = FrameNumberConfig.getFrameNumber();
        ...
        // 0x04 步进
        int type = CabinetConstants.CabinetBType.STEP.num;
        ...
        log.info("{}柜-{}：{}", request.getWorkMode().desc, CabinetConstants.CabinetBType.STEP.desc,HexUtil.format(stringBuilder.toString().toUpperCase()));
        //给B控制板 发送消息
        nettySendService.sendMsg(request.getWorkMode(),bytes,frameNumberNow);
